...
//TODO:调试define，正式使用请注释
#define __QDEBUG_CONFIG_
...
bool MainWidget::receive_data()
{
    ...
    auto str = QString(resArray.toHex());
#ifdef __QDEBUG_CONFIG_
    qDebug() << str.size() << str.constData();
#endif
    ...
    auto res=modbus.getRes();
    ui->textEditData1->setText(QString::number(res[0], 10, 4));
    ui->textEditData2->setText(QString::number(res[1], 10, 4));
    ui->textEditData3->setText(QString::number(res[2], 10, 4));
    ui->textEditData4->setText(QString::number(res[3], 10, 4));
    ui->textEditData5->setText(QString::number(res[4], 10, 4));
    return true;
}
/*-----------------关闭串口-----------------*/
